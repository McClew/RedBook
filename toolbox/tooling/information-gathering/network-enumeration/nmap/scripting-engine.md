...
```bash
clue@machine[~]$ sudo nmap <target> -sC
```

## **Specific Scripts Category**

```bash
clue@machine[~]$ sudo nmap <target> --script <category>
```

## **Defined Scripts**

```bash
clue@machine[~]$ sudo nmap <target> --script <script-name>,<script-name>,...
```

## **Specifying Scripts**

```shell-session
clue@machine[~]$ sudo nmap 10.129.2.28 -p 25 --script banner,smtp-commands
...
We see that we can recognize the **Ubuntu** distribution of Linux by using the `banner` script. The `smtp-commands` script shows us which commands we can use by interacting with the target SMTP server. In this example, such information may help us to find out existing users on the target.
...
```shell-session
McClewR@htb[/htb]$ sudo nmap 10.129.2.28 -p 80 -A
...
```bash
clue@machine[~]$ sudo nmap 10.129.2.28 -p 80 -sV --script vuln 
